...
anova(modelo_sem_categoricas, modelo)

## MMQP ----
e <- residuals(modelo_completo_log)
yhat <- fitted(modelo_completo_log)

modelo_aux <- lm(abs(e) ~ yhat)

s_hat <- fitted(modelo_aux)

s_hat[s_hat <= 0] <- min(s_hat[s_hat > 0])
...
e_mmqp <- residuals(modelo_mmqp)

e_ponderado <- sqrt(pesos) * e_mmqp
