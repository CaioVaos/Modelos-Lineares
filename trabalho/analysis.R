...
df <- df_0 %>% 
  select(-c(link, director, writer, star, filming_location, win))
...
df <- df_0 %>%
  ...
  select(-horas, -minutos, -duration)

df <- na.omit(df)
glimpse(df)
# Modelos ----------------------------------------------------------------------
modelo <- lm(df$rating_imdb ~ df$budget + df$gross_world_wide + df$nomination + df$duration_min)
summary(modelo)
